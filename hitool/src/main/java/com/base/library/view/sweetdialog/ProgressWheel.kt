...
class ProgressWheel : View {
    ...
    private fun parseAttributes(a: TypedArray) {
        ...
        barWidth = TypedValue.applyDimension(
            ...
            barWidth.toFloat(), metrics
        ).toInt()
        rimWidth = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            rimWidth.toFloat(), metrics
        ).toInt()
        circleRadius = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP, circleRadius.toFloat(), metrics
        ).toInt()

        circleRadius = a.getDimension(
            R.styleable.ProgressWheel_SAD_matProg_circleRadius, circleRadius.toFloat()
        ).toInt()

        fillRadius = a.getBoolean(
            R.styleable.ProgressWheel_SAD_matProg_fillRadius,
            false
        )

        barWidth = a.getDimension(
            R.styleable.ProgressWheel_SAD_matProg_barWidth, barWidth.toFloat()
        ).toInt()

        rimWidth = a.getDimension(
            R.styleable.ProgressWheel_SAD_matProg_rimWidth, rimWidth.toFloat()
        ).toInt()

        val baseSpinSpeed = a
            .getFloat(
                R.styleable.ProgressWheel_SAD_matProg_spinSpeed,
                spinSpeed / 360.0f
            )
        ...
        barColor = a.getColor(
            R.styleable.ProgressWheel_SAD_matProg_barColor,
            barColor
        )

        rimColor = a.getColor(
            R.styleable.ProgressWheel_SAD_matProg_rimColor,
            rimColor
        )
